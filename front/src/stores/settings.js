...
export const useSettingsStore = defineStore('settings', {
  state: () => ({
    settings: {
      ...
      mapEmbedUrl: '',
      seo: {
        title: 'MobiMax | Professional Radio Systems',
        description: 'Elite radio communication equipment.',
        keywords: 'ratsiya, walkie talkie'
      },
      pixels: {
        googleAnalyticsId: '',
        metaPixelId: '',
  ...
  actions: {
    ...
    initializePixels() {
      const { pixels } = this.settings;
      if (!pixels) return;

      // 1. Google Analytics
      if (pixels.googleAnalyticsId) {
        const gaScript = document.createElement('script');
        gaScript.src = `https://www.googletagmanager.com/gtag/js?id=${pixels.googleAnalyticsId}`;
        gaScript.async = true;
      ...
      if (pixels.googleTagManagerId) {
        ...
        gtmInit.innerHTML = `
          ...
          })(window,document,'script','dataLayer','${pixels.googleTagManagerId}');
        `;
      ...
      if (pixels.yandexMetricaId) {
        ...
        ymInit.innerHTML = `
          ...
          ym(${pixels.yandexMetricaId}, "init", {
               clickmap:true,
               trackLinks:true,
        ...
        `;
        document.head.appendChild(ymInit);
      }
    }
  }
});
